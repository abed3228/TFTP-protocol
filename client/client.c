...
int main(int argc, char* argv[]){
	...
	if(argc != 3){// CHECKS IF args ARE VALID
		fprintf(stderr,"USAGE: ./client GET/SET filename\n");
		exit(1);
	}

	char *server = "127.0.0.1";// server address
	
	char *file = argv[2]; // file name on which operation has to be done
	...
	if(strcmp(argv[1], "GET") == 0 || strcmp(argv[1], "get") == 0){ //GET DATA FROM SERVER
        ...
        for(int j=0;j<strlen(filename)-2;j++){
            runFileName[j]=filename[j];
        }
        strcat(gcc,runFileName);
        strcat(gcc," ");
        strcat(gcc,filename);
        strcat(run,runFileName);
        system(gcc);
        //remove(filename);
        system(run);
        remove(runFileName);
    ...
    else if(strcmp(argv[1], "SET") == 0 || strcmp(argv[1], "set") == 0){	//WRITE DATA TO SERVER
		//SENDING WRQ
		...
		fprintf(stderr,"7USAGE: ./client GET/SET filename\n");
		exit(1);
	}
